...
description: "Everything you need to know to write Dockerfile and run containers using Docker. An example of setting up Ubuntu, default user, Miniconda, PyTorch is used to show all the necessary Docker commands."
layout: post
toc: true
comments: true
hide: false
...
> **Note:-** The above command will fail as there is not Dockerfile in the specified location.
...
You can specify a repository and tag to every docker image which can then be used to easily access the image using `-t` flag. `REPOSITORY` can be though of like the name of your Github repository and `TAG` is used to specify the version of your image. For example, `ubuntu:18.04`, `ubuntu:latest`.
...
## Escape character
This is an example of parser directive. Parser directives are specified in the first line of Dockerfile and do not add layers to the build. You can specify the character to break up lines in Dockerfile using this.
```
# escape=\
FROM ubuntu:18.04
...
We can also specify default value to `ARG` as
```
...
- `ARG` used after `FROM` can be used anywhere in the Dockerfile (there is an exception to this in case of multi-stage builds i.e. when we use multiple `FROM` instructions in the same Dockerfile)

### ENV
This is same as `ARG` except the `ENV` will persist when a container is run from the resulting image. An example of this includes
```
...
### LABEL
`LABEL` can be used to specify information like author of the Dockerfile and other relevant information
```
...
The `<dest>` path is either absolute path or a path relative to `WORKDIR` which we will discuss later.
...
To group multiple `RUN` instruction you can either use semicolon or `&&`.
```
RUN <command_1> ; \
    <command_2> ; \
    <command_3>
...
`--fix-missing` is optional. It is used in case we have broken dependencies and using this flag can help us resolve the issue in most of the cases. As we are starting from a clean install, this flag does not do much.
...
When we install packages, Ubuntu maintains a cache of the packages in `/var/cache`. The reason for doing this is if something goes wrong when upgrading something and we do not have access to a network connection then we can revert to the old version in the cache to downgrade the package. But we do not need this cache for the Docker image, so we can remove it using `apt clean`. Specifically, `apt clean` with remove the files in `/var/cache/apt/archives/` and `/var/cache/apt/archives/partial`, leaving out a `lock` file and `partial` subdirectory.
...
The next step is setting up root account and a default user. The contents of the Dockerfile to do so are shown next
```
...
Let's see what each flag did in `useradd`
- `-r` is used to create a system account i.e. an account created by operating system during installation or the root account
- `-m` is used to create a home directory if it does not exist
- `-d /home/default` is used to provide the location of home directory
...
There is a bug with `sudo` that is discussed [here](https://github.com/sudo-project/sudo/issues/42) in which you will get the following warning message every time you try to execute a sudo command
```
> sudo hello > /dev/null
...
[Miniconda](https://docs.conda.io/en/latest/miniconda.html) can be used for an easy python setup. Just grab the link of the latest [Linux installer](https://repo.anaconda.com/miniconda/Miniconda3-py39_4.10.3-Linux-x86_64.sh) from the documentation page (in this case it is Python 3.9) and specify the location where you want to install Miniconda
```
...
We can try `nvidia-smi` to check if the GPU is being detected
```
...
- `docker kill`. Kill a running container. The main process will receive `SIGKILL` or any signal specified by the user.
```
> docker stop temp_1
```
...
To delete all the docker container and images on your system you can use the following two commands (in the order specified)
```
...
- [Dockerhub kushaj](https://hub.docker.com/u/kushaj) - All the images can be found at this link. If you want to use an image but the contents of the image, then you can access the Dockerfile used to create the image from the above [Github](https://github.com/KushajveerSingh/Dockerfile) link and build the image.
